defmodule Raxx.Session.SignedCookieTest do
  ...
  test "Error for no cookies sent", %{config: config} do
    assert {:error, :no_cookies_sent} =
             Raxx.request(:GET, "/")
             |> SignedCookie.extract(config)
  end

  test "Error for no session cookie sent", %{config: config} do
    assert {:error, :no_session_cookie} =
             Raxx.request(:GET, "/")
  ...
  test "Error for unsigned session", %{config: config} do
    assert {:error, :invalid_session_cookie} =
             Raxx.request(:GET, "/")
  ...
  test "Error for tampered session", %{config: config} do
    assert {:error, :could_not_verify_signature} =
             Raxx.request(:GET, "/")
             |> Raxx.set_header(
               "cookie",
               "raxx.session=g2gCZAAEdXNlcmEZ--5jKDb7R4i5HOq9ZSbeIAxB4GaO5RXfkOKm4CX7YSeJk="
             )
             |> SignedCookie.extract(config)
  end
end
